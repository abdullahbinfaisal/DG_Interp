...
class R(Processor):
    ...
    def calculate_r_scores(self):
        ...
        Returns:
            r_mag:
                ...
                entropy over |D_d(k,c)| across domains

            discrimination_scores:
                shape (classes, concepts, domains)
                signed domain-wise D_d(k,c), saved as R_acts
        ...
        eps = 1e-12
        nb_domains = discrimination_scores.shape[2]
        log_m = torch.log(
            torch.tensor(float(nb_domains), device=discrimination_scores.device)
        )

        # Magnitude-based domain distribution:
        # r_d = |D_d(k,c)| / sum_d |D_d(k,c)|
        mag = discrimination_scores.abs()
        denom = mag.sum(dim=2, keepdim=True)

        probs = mag / denom.clamp(min=eps)

        r_mag = -1.0 / log_m * (probs * torch.log(probs + eps)).sum(dim=2)

        # If a concept has zero discriminative effect in every domain,
        # its R score should be zero, not NaN or arbitrary.
        zero_mask = denom.squeeze(2) <= eps
        r_mag[zero_mask] = 0.0

        return (
            r_mag.contiguous(),
            discrimination_scores.contiguous(),
            counts.contiguous(),
        )

    def process(self):
        r_mag, disc_scores, counts = self.calculate_r_scores()

        # R is the magnitude-consistency score. Sign consistency is deliberately
        # not stored: it is recoverable from R_acts if ever needed, and the paper
        # commits to a three-score framework.
        self.dump(r_mag, name="R")
        self.dump(disc_scores, name="R_acts")
        self.dump(counts, name="R_counts")
    ...
    def _calculate_discrimination_scores_per_domain(self):
            ...
            for x, y in tqdm(dataloader, desc=f"R scores [{domain}]"):
                ...
                with torch.no_grad():
                    ...
                    for c in active_concepts_batch:
                        ...
                        accumulated_scores[:, c, d].scatter_add_(
                            0,
                            active_classes,
                            active_drops
                        )
                        activation_counts[:, c, d].scatter_add_(
                            0,
                            active_classes,
        ...
        return (
            accumulated_scores / activation_counts.clamp(min=1),
            activation_counts
        )